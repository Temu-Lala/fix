...
export default function ChatScreen() {
  ...
  try {
    if (product) productObj = JSON.parse(product);
  } catch {}
  const [showProduct, setShowProduct] = useState(!!productObj);
  ...
  const startRecording = async () => {
    if (Platform.OS === "web") {
      Alert.alert(
        "Voice Recording",
        "Voice recording is not available on web platform"
      );
      return;
    }
  ...
  const handleVideoCall = () => {
    if (Platform.OS === "web") {
      Alert.alert(
        "Video Call",
        "Video calls are not available on web platform"
      );
      return;
    }
    setCallType("video");
  ...
  const handleVoiceCall = () => {
    if (Platform.OS === "web") {
      Alert.alert(
        "Voice Call",
        "Voice calls are not available on web platform"
      );
      return;
    }
    setCallType("voice");
    ...
    >
      ...
      {showProduct && productObj && (
        <View style={styles.productPreviewCard}>
          <TouchableOpacity
            style={styles.productPreviewClose}
            onPress={() => setShowProduct(false)}
          >
            <X size={18} color="#888" />
          </TouchableOpacity>
          <Image
            source={{ uri: productObj.image }}
            style={styles.productPreviewImg}
          />
          <View style={styles.productPreviewInfo}>
            <Text style={styles.productPreviewTitle} numberOfLines={1}>
              {productObj.title}
            </Text>
            <Text style={styles.productPreviewDesc} numberOfLines={2}>
              {productObj.description}
            </Text>
            <TouchableOpacity
              style={[
                styles.productPreviewBtn,
                { backgroundColor: colors.primary },
              ]}
              onPress={() => router.push(`/barter/${productObj.id}`)}
            >
              <Text
                style={[styles.productPreviewBtnText, { color: colors.white }]}
              >
                View Details
              </Text>
            </TouchableOpacity>
              ...
              <View style={styles.callControls}>
                ...
                {callType === "video" && (
                  <TouchableOpacity
                    style={[
                      styles.callControlButton,
                      isVideoOff && { backgroundColor: colors.error },
                    ]}
                    onPress={toggleVideo}
                  >
                    {isVideoOff ? (
